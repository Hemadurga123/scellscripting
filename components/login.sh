...
cd /home/ubuntu && rm -rf login && apt install unzip && && cd /tmp && unzip -o /tmp/login.zip &>>$LOG && mv login-main login && cd /home/ubuntu/login && export GOPATH=/home/ubuntu/go && export GOBIN=$GOPATH/bin && go get &>>$LOG && go build
Check $?
